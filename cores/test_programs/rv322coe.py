...
def execute(args) -> None:
    ...
    make_elf = (
        "riscv64-unknown-elf-ld -march=rv32i -melf32lriscv -Ttext=0x0 -o "
        + str(file_elf)
        + " "
        + str(file_o)
    )
    ...
    for command in commands:
        try:
            ...
            raise RuntimeError(
                "Couldn't run: {}, command returned: {}".format(command, cpe)
            )

    # Convert .hex to .coe
    try:
        with (
            open(file_tmp_hex, "r") as hex_file,
            open(file_coe, "w") as coe_file,
            open(file_hex, "w") as new_hex_file,
        ):
            coe_file.write(
                ...
                if not line.startswith("@"):
                    ...
                    for i in range(0, len(bytes), 4):
                        if i + 3 < len(bytes):  # Ensure 4 bytes for an instruction
                            ...
                            word = (
                                f"{bytes[i + 3]}{bytes[i + 2]}{bytes[i + 1]}{bytes[i]}"
                            )
    ...
    if args.verbosity:
        print(
            "Successfully created {} with {} instructions".format(
                file_coe, instruction_count
            )
        )
