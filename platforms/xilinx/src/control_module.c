#include "control_module.h"
#include "control_definitions.h"
#include "platform.h"
#include "xbram.h"
#include "xil_io.h"
#include <stdint.h>


void cm_regfile_write(uint8_t register_num, int value) {
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR + REGFILE_ADDR(register_num);
  Xil_Out32(address, value);
}

uint32_t cm_regfile_read(uint8_t register_num) {
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR + REGFILE_ADDR(register_num);
  uint32_t read_value = Xil_In32(address);

  return read_value;
}

void cm_pc_set(uint32_t value) {
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR + CONTROL_REG_ADDR(CTRL_REG_PC);
  Xil_Out32(address, value);
}

uint32_t cm_pc_read() {
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR + CONTROL_REG_ADDR(CTRL_REG_PC);
  uint32_t pc_value = Xil_In32(address);

  return pc_value;
}

void cm_single_step_core() {
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR + CONTROL_REG_ADDR(CTRL_REG_STEP);
  uint32_t step_trigger_val = 0x01;
  Xil_Out32(address, step_trigger_val);
}

void cm_core_start() {
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR + CONTROL_REG_ADDR(CTRL_REG_START);
  uint32_t start_trigger_val = 0x01;
  Xil_Out32(address, start_trigger_val);
}

void cm_core_stop() {
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR + CONTROL_REG_ADDR(CTRL_REG_STOP);
  uint32_t stop_trigger_val = 0x01;
  Xil_Out32(address, stop_trigger_val);
}

uint32_t cm_debug_vector_read(){
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR + CONTROL_REG_ADDR(CTRL_REG_DBG_VECTOR);
  return Xil_In32(address);
}

uint32_t cm_fault_request(uint8_t register_num, cm_fault_mode_t mode) {
  return FAULT_REQUEST(mode, register_num);
}

void cm_regfile_fault_inject(uint8_t register_num, uint32_t mask,
                             cm_fault_mode_t mode) {
  uint32_t address = XPAR_RISCV_MOD_NAME_BASEADDR +
                     cm_fault_request(register_num, mode);
  Xil_Out32(address, mask);
}
